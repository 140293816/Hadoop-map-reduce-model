...
public class RankSortMapper extends Mapper<LongWritable, Text, FloatWritable, Text> {
    ...
    {
    	//Split Text objects in the form: [page] [pagerank] [outlinks], if the page contains outlinks.
    	String[] ranks=value.toString().split("\t");
    	
    	//Ignore invalid message
    	if(ranks.length<2)return;
    	
    	//record valid message to the map in the form: [pageRank], [Page]
    	context.write(new FloatWritable(Float.parseFloat(ranks[1])), new Text(ranks[0]));
